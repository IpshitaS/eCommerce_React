        ...
        <div className="p-5 m-2 text-xl">
          ...
          <span>
            ...
            <span className="text-xs">
              , {restaurant?.cards[0]?.card?.card?.info?.totalRatings / 1000}k
            </span>
          </span>
          ...
          <div>
            {restaurant?.cards[2]?.groupedCard?.cardGroupMap?.REGULAR?.cards.map(
              (Cards, index) => (
                <div
                  key={index}
                  className="rounded-t-lg border border-neutral-200 bg-white dark:border-neutral-600 dark:bg-neutral-800 mx-40"
                >
                  {!isVisible ? (
                    <button
                      onClick={() => setIsVisible(true)}
                      isVisible={visableSection == { index }}
                      setIsVisable={() => setVisableSection({ index })}
                          ...
                          >
                            ...
                            <div className="">
                              <img
                                className="w-20"
                                src={
                                  img_CDN_URL + itemCards?.card?.info.imageId
                                }
                              />
                              <button
                                data-testid="add-Btn"
                                ...
                                onClick={() =>
                                  addFoodItem(itemCards?.card?.info)
                                }
                              >
                                Add
                              </button>
